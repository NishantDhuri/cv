...
image = cv2.imread('D:/Nishant/MSC-IT/cv_new/input/photo.png')
...
plt.axis('off')
plt.show()
scaling_matrix = np.float32([[1.1, 0, 0], [0, 1.1, 0]])
...
affline_matrix = cv2.getAffineTransform(original_points, transformed_points)
affline_transformed_image = cv2.warpAffine(image, affline_matrix, (width, height))
plt.figure(figsize=(6, 6))
plt.imshow(cv2.cvtColor(affline_transformed_image, cv2.COLOR_BGR2RGB))
plt.title('Affline Transformed Image')
plt.axis('off')
plt.show()
...
perspective_transformed_image = cv2.warpPerspective(image, perspective_matrix, (width,
height))
plt.figure(figsize=(6, 6))
